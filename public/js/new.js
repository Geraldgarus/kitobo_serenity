      ...
      detailBody.innerHTML = `
        ...
        ${isActive ? `
          <div style="margin-top:16px;padding:12px;background:#e8f5e9;border-radius:8px;text-align:center;">
            <span style="font-size:12px;">🟢 Currently staying - Click Checkout to make room available</span>
          </div>
        ` : `
          <div style="margin-top:16px;padding:12px;background:#f5f5f5;border-radius:8px;text-align:center;">
            <span style="font-size:12px;">✅ Reservation completed - Guest checked out</span>
          </div>
        `}
      `;
    }
...
async function checkoutReservation() {
  ...
  try {
    ...
    showToast(`✅ ${selectedReservation.guest} checked out successfully!\nRoom is now available. New total: ${fmtTSH(newTotal)}`, '🚪');
...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  try {
    ...
    showToast('⚠️ Cannot reach API server. Check that the backend is running.', '❌');
  }
});
